...
export function Footer() {
  ...
  return (
    <footer className="bg-gradient-to-b from-[#111A2E] to-[#0B1426] border-t border-border/50 pt-24 pb-12 relative overflow-hidden">
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-primary/95 backdrop-blur-sm border border-primary-foreground/20 rounded-[2.5rem] p-8 md:p-12 mb-20 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-primary/20 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-2">Ready to transform your business?</h3>
            <p className="text-muted-foreground text-lg">Let's build something extraordinary together.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16">
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="flex gap-3">
              <Button variant="outline" size="icon" className="rounded-full border-border text-muted-foreground hover:bg-foreground/10 dark:bg-white/10 hover:text-foreground bg-transparent transition-colors">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-foreground font-bold tracking-wide">Products</h3>
            <ul className="space-y-4">
              {(footerProducts.length ? footerProducts : FOOTER_FALLBACK).slice(0, 8).map((p) => (
                <li key={p.key}>
                  <Link href={`/products#${p.key}`} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {p.title}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-foreground font-bold tracking-wide">Company</h3>
            <ul className="space-y-4">
              <li><Link href="/" className="text-sm text-muted-foreground hover:text-primary transition-colors">About Us</Link></li>
              <li><Link href="/demo" className="text-sm text-muted-foreground hover:text-primary transition-colors">Live Demos</Link></li>
          ...
          <div className="lg:col-span-4 space-y-6">
            <h3 className="text-foreground font-bold tracking-wide">Contact Us</h3>
            <ul className="space-y-5">
              {locations.length > 0 ? (
                locations.slice(0, 3).map((loc) => {
                  const lines = formatLocationAddress(loc);
                  return (
                    <li key={loc.id} className="flex items-start gap-4 text-sm text-muted-foreground" data-testid={`footer-location-${loc.id}`}>
                      <div className="w-10 h-10 rounded-full bg-foreground/5 dark:bg-white/5 flex items-center justify-center shrink-0">
                        <MapPin className="w-4 h-4 text-primary" />
                      </div>
                      <div className="pt-1.5">
                        <div className="text-foreground/90 font-semibold leading-tight">
                          {loc.label}
        ...
        <div className="pt-8 border-t border-border/50 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            <Link href="/privacy" className="text-sm text-foreground/40 hover:text-foreground transition-colors">Privacy Policy</Link>
            <Link href="/terms" className="text-sm text-foreground/40 hover:text-foreground transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
